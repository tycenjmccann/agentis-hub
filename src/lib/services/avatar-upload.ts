...
/**
 * Service layer for avatar upload and delete operations.
 * Handles multipart parsing, S3 upload/delete, DynamoDB update, and previous avatar cleanup.
 */
export class AvatarUploadService {
   ...
   * Sets Cache-Control for CDN caching (SEC-SERVE-06).
  ...
  async uploadAvatar(userId: string, file: ParsedFile, format: ImageFormat): Promise<string> {
    ...
    // Upload to S3 with Cache-Control and Content-Type metadata
  ...
  /**
   * Delete avatar from S3 and clear the user's avatar fields in DynamoDB.
   * Returns the S3 key that was deleted (for audit logging).
   */
  async deleteAvatar(userId: string): Promise<{ deleted: boolean; previousKey?: string }> {
    // Get current avatar key
    const getResult = await this.dynamoClient.send(
      new GetCommand({
        TableName: this.config.tableName,
        Key: { PK: `USER#${userId}`, SK: 'PROFILE' },
        ProjectionExpression: 'avatarS3Key, avatarUrl',
      })
    );

    const avatarS3Key = getResult.Item?.avatarS3Key;
    if (!avatarS3Key) {
    ...
    // Clear avatar fields in DynamoDB
    await this.dynamoClient.send(
      new UpdateCommand({
        TableName: this.config.tableName,
        Key: { PK: `USER#${userId}`, SK: 'PROFILE' },
        UpdateExpression: 'REMOVE avatarUrl, avatarS3Key SET updatedAt = :now',
        ExpressionAttributeValues: {
          ':now': new Date().toISOString(),
        },
      })
    ...
    return { deleted: true, previousKey: avatarS3Key };
  }
}
